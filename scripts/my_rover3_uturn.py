...
SPEED = 1590 #initial speed set to stationary
drd_rps = 12.0 #desired rpm for the robot to move at. This is the setpoint for controller
vel_ctrl_effort = 0
...
def move_forward():
	...
	# if time.time() - tt >=3:
	# 	tt  = time.time()
	# 	turn_plus = turn_plus + 50
	# turn_amt = turn_plus
	if turn_amt > MAX_TURN:
		turn_amt = MIN_TURN#MAX_TURN
		# turn_plus = turn_amt
...
def explore():
	...
	# t0 += time.time() - t1
	#t1 = time.time()
	print time.time()-t1,msg.channels[throttle],'\t',msg.channels[steer],'\t',rps,'\t',my_heading
	# print t0,gyro_drift
...
def listener():
	...
	if True:	
		header = ','.join(['time','throttle','rps','drd_rps','vel_control_effort','steering','my_heading','drd_heading','hdg_ctrl_effort\n'])
	...
	#		print yaw,'\t',raw_yaw,'\t',my_heading
			...
			if cmd=='e':
				if hdg_set:
					explore()
					j = ','.join(map(str,[time.time()-t1,msg.channels[throttle],rps,drd_rps,round(vel_ctrl_effort,2),msg.channels[steer],my_heading,drd_heading,round(hdg_ctrl_effort,2)]))
					# a = angles.shortest_angular_distance(pre_yaw,raw_yaw)
					# pre_yaw = raw_yaw
					# a = angles.to_degrees(a)
					# gyro_drift += a/math.pi * 180
					# j = ','.join(map(str,[abs(gyro_drift),t0]))
					# d.write(j + '\n')
					data = j + '\n'
				...
				Action = 'Teleoperated'
				msg = deepcopy(msg1)
				j2 = ','.join(map(str,[time.time()-t1,msg.channels[throttle],rps,drd_rps,round(vel_ctrl_effort,2),msg.channels[steer],my_heading,drd_heading,round(hdg_ctrl_effort,2)]))
				# t.write(j2 + '\n')
